...
const cargarFotografia = async (req, res) => {
    ...
    let partiturasId = req.params.id;
    ...
    try {
        const partiturasActualizada = await partituras.findOneAndUpdate(
            { _id: partiturasId },
            {
                $set: {
...
const borrarPartituras = async (req, res) => {
    ...
    try {
        let part = await partituras.findOneAndDelete({ _id: id });
...
const editarPartituras = async (req, res) => {
    ...
    try {
        let part = await partituras.findByIdAndUpdate(id, datosActualizados, { new: true });
